...
contract EscrowSeamTest is Test {
    ...
    function setUp() public {
        ...
        seam.transfer(address(esSEAM), 50_000_000 ether);
        seam.transfer(address(user), 50_000_000 ether);
    }

    function testDeploy() public {
        assertEq(esSEAM.vestingDuration(), VESTING_DURATION);
        assertEq(esSEAM.seam(), address(seam));
        assertEq(esSEAM.owner(), address(this));
    }

    function testSimpleDeposit() public {
        uint256 depositorBalanceBefore = seam.balanceOf(address(user));
        uint256 esSEAMBalanceBefore = seam.balanceOf(address(esSEAM));
