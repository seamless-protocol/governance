...
import {IAToken} from "static-a-token-v3/src/interfaces/IAToken.sol";
import {ERC20} from "static-a-token-v3/src/ERC20.sol";
import {IInitializableStaticATokenLM} from "static-a-token-v3/src/interfaces/IInitializableStaticATokenLM.sol";
import {StaticATokenErrors} from "static-a-token-v3/src/StaticATokenErrors.sol";
import {RayMathExplicitRounding, Rounding} from "static-a-token-v3/src/RayMathExplicitRounding.sol";
import {IERC4626} from "static-a-token-v3/src/interfaces/IERC4626.sol";
 ...
 * @author BGD labs
 */
contract StaticATokenLMUpgrade is
    Initializable,
    ...
    IERC4626
{
    ...
    function initializeV2() external reinitializer(2) {
        for (uint256 i = 0; i < _rewardTokens.length; i++) {
            _startIndex[_rewardTokens[i]].lastUpdatedIndex = _startIndex[_rewardTokens[i]].lastUpdatedIndex_deprecated;
        }
    }
    ...
    function metaDeposit(
        ...
        if (permit.deadline != 0) {
            try IERC20WithPermit(depositToAave ? address(_aTokenUnderlying) : address(_aToken)).permit(
        ...
        unchecked {
            bytes32 digest = keccak256(
                abi.encodePacked(
                    ...
                    keccak256(
                        abi.encode(
                            METAWITHDRAWAL_TYPEHASH, owner, receiver, shares, assets, withdrawFromAave, nonce, deadline
                        )
                    )
                )
            );
    ...
    function _withdraw(address owner, address receiver, uint256 _shares, uint256 _assets, bool withdrawFromAave)
        internal
        returns (uint256, uint256)
    {
        require(receiver != address(0), StaticATokenErrors.INVALID_RECIPIENT);
        ...
        returns (uint256)
    {
        ...
        if (currentUserRewardsData.rewardsIndexOnLastInteraction == 0) {
            currentUserRewardsData.rewardsIndexOnLastInteraction =
                currentUserRewardsData.rewardsIndexOnLastInteraction_deprecated;
        }
    ...
    function _registerRewardToken(address reward) internal {
        ...
        _startIndex[reward] = RewardIndexCache(true, 0, startIndex);
