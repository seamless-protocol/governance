...
abstract contract RewardsDistributor is IRewardsDistributor {
    ...
    function getAssetIndex(address asset, address reward) external view override returns (uint256, uint256) {
        ...
        return _getAssetIndex(rewardData, IScaledBalanceToken(asset).scaledTotalSupply());
    }
        ...
        onlyEmissionManager
    {
        ...
        for (uint256 i = 0; i < rewards.length; i++) {
            ...
            (uint256 newIndex,) = _updateRewardData(rewardConfig, IScaledBalanceToken(asset).scaledTotalSupply());
    ...
    function _configureAssets(RewardsDataTypes.RewardsConfigInput[] memory rewardsInput) internal {
        for (uint256 i = 0; i < rewardsInput.length; i++) {
            ...
            (uint256 newIndex,) = _updateRewardData(rewardConfig, rewardsInput[i].totalSupply);
     ...
     *
     */
    function _updateRewardData(RewardsDataTypes.RewardData storage rewardData, uint256 totalSupply)
        internal
    ...
    function _updateData(address asset, address user, uint256 userBalance, uint256 totalSupply) internal {
        ...
        unchecked {
            for (uint128 r = 0; r < numAvailableRewards; r++) {
                ...
                (uint256 newAssetIndex, bool rewardDataUpdated) = _updateRewardData(rewardData, totalSupply);
        ...
        returns (uint256)
    {
        ...
        return _getRewards(userAssetBalance.userBalance, nextIndex, userIndex);
    }
     ...
     *
     */
    function _getRewards(uint256 userBalance, uint256 reserveIndex, uint256 userIndex)
        internal
        pure
        returns (uint256)
    {
     ...
     *
     */
    function _getAssetIndex(RewardsDataTypes.RewardData storage rewardData, uint256 totalSupply)
        internal
        view
